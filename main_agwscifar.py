...
# dataset settings
parser.add_argument('--noise_mode', default=NOISE_MODE.AGGRE, type=str, help='artifical noise mode (default: symmetric)')
parser.add_argument('--noise_ratio', default=0.0, type=float, help='artifical noise ratio (default: 0.5)')
...
def main():
    ...
    test_loader = torch.utils.data.DataLoader(test_data, batch_size=args.batch_size, shuffle=False, num_workers=1, pin_memory=True)
    eval_loader = torch.utils.data.DataLoader(eval_data, batch_size=args.batch_size, shuffle=False, num_workers=1, pin_memory=True)
    all_loader = torch.utils.data.DataLoader(all_data, batch_size=args.batch_size, num_workers=1, shuffle=True, pin_memory=True, drop_last=True)
    ...
    for i in range(args.epochs):
        ...
        clean_candidate_id_filtered = raw_clean_candidate_ids[exclude_mask]
        clean_candidate_ids = torch.unique(torch.cat((clean_candidate_id_filtered, clean_id_extended)))

        mask = ~torch.isin(raw_undecided_ids, clean_id_extended)
